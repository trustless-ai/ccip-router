  ...
  <div class="audit-panel" id="deploy-panel" style="margin-top:16px">
    ...
    <div id="deploy-body" style="display:none">
      <div class="config-form">
        ...
        <div class="config-section">
          <div class="config-section-title">One-time setup — no private key stored</div>
          <p style="font-size:13px;color:var(--text-muted);margin:0 0 12px">
            Deploy <strong>AttestationIndex</strong> and <strong>NodeRegistry</strong> to any EVM chain directly from your browser wallet.
            Your key never leaves MetaMask. Addresses are saved to config automatically after deployment.
          </p>
...
<script>
  ...
  async function deployContracts() {
    ...
    try {
      ...
      status.textContent = \`Connected: \${account.slice(0,6)}…\${account.slice(-4)} · chain \${chainId}\`
      ...
      // Auto-save addresses to config and restart
      const cfgRes = await fetch('/admin/api/config', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ attestationIndex: attestAddr, nodeRegistry: registryAddr }),
      })
      if (cfgRes.ok) {
